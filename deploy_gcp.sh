...
echo "[1/4] Configurando cuenta y proyecto..."
...
if [ -n "$POSTGRES_DB_PASSWORD" ]; then
  echo ""
  echo "[2/4] Contraseña de Cloud SQL detectada en .env.secrets."
  POSTGRES_PASSWORD="$POSTGRES_DB_PASSWORD"
else
  echo ""
  echo "[2/4] Ingresá la contraseña del usuario postgres de Cloud SQL:"
...
echo ""
echo "[3/4] Desplegando $SERVICE_NAME en Cloud Run ($REGION)..."
